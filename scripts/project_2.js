...
///Takes in input from the text box and sends it to the mapbox API to produce the Geo-Cordinates.
$("#button_id").on('click', function () {
	...
	const addMarkerAndPopup=(latlng)=>{
		...
		if(latlng) {	//"latlng" will be undefined if the geocoding fails.
			...
			if(BinarySearch2dArray(latlng,locations)==-1) {	//Only add a new marker if a marker doesn't exists
				...
				let p=$('<p></p>');
				p.append(($("#input_id").val()).toUpperCase());
	...
	const add_Location_To_Side_bar = (err, data) => {
		...
		if (data.latlng) {
			//Create paragraph element with lat & lng attributes.
			let p=$('<p></p>');
			p.append($("#input_id").val().toUpperCase() + ":  " + data.latlng);
			p.attr('class','data_class italics');
			p.attr('lat',data.latlng[0]);
			p.attr('lng',data.latlng[1]);
			$("#records_id").append(p);

			//Add click handler to every new paragraph added.
			p.click(function(event){
				mymap.flyTo([$(this).attr('lat'),$(this).attr('lng')], 8);
			});			
